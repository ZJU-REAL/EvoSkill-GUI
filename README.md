...
CoEvoSkill is a training-free framework for mobile GUI agents that retrieves or
creates task skills, executes them in MobileWorld, diagnoses failed rollouts with
an information-isolated verifier, and refines the skills for another attempt.
The executor, verifier, and skill editor can share the same model backbone.

This repository is built on
[MobileWorld](https://github.com/Tongyi-MAI/MobileWorld) and keeps its benchmark,
runtime, and CLI. CoEvoSkill adds structured skill packages, retrieval and
reranking, failure-driven evolution, and accessibility-tree support.

## CoEvoSkill Quick Start

Requirements are the same as MobileWorld: Linux, Docker with privileged-container
support, KVM, Python 3.12, and `uv`.
...
Set an OpenAI-compatible endpoint, then run one of the provided configurations:
...
The main integration can also be invoked directly:
...
uv run mw eval \
  ...
  --log_file_root traj_logs/coevoskill \
  --enable_evolution \
...
Generated trajectories, skill stores, reports, credentials, virtual environments,
and local train/test splits are intentionally excluded from version control.

## Repository Layout

| Path | Purpose |
| --- | --- |
| `src/mobile_world/skills/` | Skill packages, retrieval, verifier, and evolution loop |
| `src/mobile_world/agents/implementations/` | CoEvoSkill executor integrations |
| `scripts/` | Supported agent launch scripts |
| `tools/a11y_tree_tool/` | Optional Android accessibility-tree package |

See [the skill-system documentation](src/mobile_world/skills/README.md) for the
architecture and CLI flags. See
[the publication checklist](docs/PUBLICATION_CHECKLIST.md) before pushing a new
public repository.
