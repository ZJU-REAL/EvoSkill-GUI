...
def _add_common_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    # ------------------------------------------------------------------
    # CoEvoSkill / self-evolving skills options
    # ------------------------------------------------------------------
    parser.add_argument(
        "--enable-evolution",
        "--enable_evolution",
        ...
        action="store_true",
        help=(
            "Enable CoEvoSkill self-evolving skill loop. "
            "Requires --agent_type=evo_skill or --agent_type=mai_ui_evo_skill."
        ),
    )
    parser.add_argument(
        "--skills-store",
        ...
        dest="skills_store",
        default=None,
        help="Directory to read/write skill packages (default: <log_file_root>/_skills_store)",
    )
    parser.add_argument(
        "--max-evolution-iterations",
        "--max_evolution_iterations",
        dest="max_evolution_iterations",
        type=int,
        default=3,
        help="Maximum number of self-evolution iterations per task (default: 3)",
    )
    parser.add_argument(
        "--retrieval-threshold",
        "--retrieval_threshold",
        dest="retrieval_threshold",
        type=float,
        default=0.6,
        help=(
            "Skill retrieval similarity threshold; below this a new skill is generated. "
            "Default 0.6 matches the BM25-only retriever (KeywordSkillRetriever)."
        ),
    )
    parser.add_argument(
        "--enable-a11y",
        ...
        action="store_true",
        help=(
            "Wrap each environment with a11y_tool to capture per-step accessibility "
            "tree data. Used by both the executor agent (skill generation, a11y_utils) "
            "and the verifier (trajectory diagnosis). Requires the a11y_tool package "
            "and ADB-accessible containers."
        ),
    )
    parser.add_argument(
        "--enable-a11y-for-agent",
        "--enable_a11y_for_agent",
        dest="enable_a11y_for_agent",
        action="store_true",
        help=(
            "Inject the latest accessibility tree (with bounds + state) into the "
            "executor agent's prompt at every step, alongside the screenshot. "
            "Implies --enable-a11y. Only effective with self-evolution agents "
            "(evo_skill / mai_ui_evo_skill)."
        ),
    )
...
async def _execute_pass_k(
    ...
    for i in range(1, pass_k + 1):
        ...
        run_agent_with_evaluation(
            ...
            enable_evolution=getattr(args, "enable_evolution", False),
            skills_store=getattr(args, "skills_store", None),
            max_evolution_iterations=getattr(args, "max_evolution_iterations", 3),
            retrieval_threshold=getattr(args, "retrieval_threshold", 0.6),
            enable_a11y_for_verifier=(
                getattr(args, "enable_a11y", False)
                or getattr(args, "enable_a11y_for_agent", False)
            ),
            inject_a11y_to_agent=getattr(args, "enable_a11y_for_agent", False),
        )
