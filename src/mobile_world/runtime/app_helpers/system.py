...
def _wait_device_ready(controller: AndroidController, timeout: int = 60) -> None:
    # pm clear may briefly knock adb / SurfaceFlinger offline (~5-30s on headless
    # x86 AVDs). Give it a chance to recover before the next caller hits
    # get_screenshot, otherwise the 3-shot retry there would mis-flag it as
    # "Device is not healthy".
    import time as _time

    deadline = _time.time() + timeout
    while _time.time() < deadline:
        r = execute_adb(f"adb -s {controller.device} shell getprop sys.boot_completed")
        if r.success and "1" in (r.output or ""):
            return
        _time.sleep(2)
